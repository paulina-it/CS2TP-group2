<form method="POST" action="{{ route('logout') }}" class="m-0 ">
    @csrf
    <div id="modal-logout" data-modal-backdrop="dynamic" tabindex="-1"
        class="modalWindow hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-100 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-4 md:p-5 text-center">
                    <h4 class="m-5">
                        Are you sure you want to log out?</h4>
                    <form action="{{ route('logout') }}" method="POST">
                        @csrf
                        {{-- <button type="submit" value="Delete"
                                                    id="red-btn">Delete</button> --}}
                        <button value="Delete" data-modal-hide="modal-logout" type="button"
